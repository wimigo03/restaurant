...
@section('content')
    @include('inicio_mes_fiscal.partials.form-create')
@endsection
@section('scripts')
    @parent
    ...
    <script>
        ...
        function validar(){
            if($("#anho").val() == ""){
                alert("[LA GESTION ES UN CAMPO OBLIGATORIO]");
                return false;
            }

            if($("#mes").val() == ""){
                alert("[EL MES DE EJERCICIO FISCAL ES UN CAMPO OBLIGATORIO]");
                return false;
            }

            if($("#pregunta_1 >option:selected").val() == ""){
                alert("[FALTA COMPLETAR UN CAMPO PARA EL REGISTRO]");
                return false;
            }
        ...
        function confirmar(){
            var url = "{{ route('inicio.mes.fiscal.store') }}";
