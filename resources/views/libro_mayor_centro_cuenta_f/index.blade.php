...
@section('breadcrumb')
    ...
    <span>Libro Mayor por Centro y Cuenta</span>
@endsection
@section('content')
    @include('libro_mayor_centro_cuenta_f.partials.search')
    @include('libro_mayor_centro_cuenta_f.partials.table')
@endsection
@section('scripts')
    @parent
    ...
    <script>
        ...
        function getCentros(id,CSRF_TOKEN){
            $.ajax({
                type: 'GET',
                url: '/libro-mayor-centro-cuenta-f/get_centros',
                data: {
                    _token: CSRF_TOKEN,
                    id: id
                },
        ...
        function getSubCentros(id,CSRF_TOKEN){
            $.ajax({
                type: 'GET',
                url: '/libro-mayor-centro-cuenta-f/get_subcentros',
                data: {
                    _token: CSRF_TOKEN,
                    id: id
                },
        ...
        function getPlanCuentas(id,CSRF_TOKEN){
            $.ajax({
                type: 'GET',
                url: '/libro-mayor-centro-cuenta-f/get_plancuentas',
                data: {
                    _token: CSRF_TOKEN,
                    id: id
                },
        ...
        function search(){
            var url = "{{ route('libro.mayor.centro.cuenta.f.search') }}";
            $("#form").attr('action', url);
            $("#form").submit();
        }

        function limpiar(){
            var url = "{{ route('libro.mayor.centro.cuenta.f.index') }}";
            window.location.href = url;
        }

        function excel() {
            var url = "{{ route('libro.mayor.centro.cuenta.f.excel') }}";
            $(".btn").hide();
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    a.href = url;
                    a.download = 'libro_mayor_centro_y_cuenta.xlsx';
        ...
        function pdf() {
            var url = "{{ route('libro.mayor.centro.cuenta.f.pdf') }}";
            $(".btn").hide();
            $(".spinner-btn-send").show();
            var form = $("#form");
            var formData = form.serialize();
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var url = window.URL.createObjectURL(response);
                    a.href = url;
                    a.download = 'libro_mayor_centro_y_cuenta.pdf';
                    document.body.appendChild(a);
                    a.click();
                    window.URL.revokeObjectURL(url);
                    $(".spinner-btn-send").hide();
        ...
        function cambiari(){
            var url = "{{ route('libro.mayor.centro.cuenta.index') }}";
            window.location.href = url;
        }
    </script>
@stop
