...
Route::prefix('tipo-precios')->name('tipo.precios.')->middleware(['auth:sanctum',config('jetstream.auth_session'),'verified',])->group(function () {
    ...
    Route::get('/search', 'TipoPrecioController@search')->name('search')->middleware('can:tipo.precios.index');
    Route::get('/create', 'TipoPrecioController@create')->name('create')->middleware('can:tipo.precios.create');
