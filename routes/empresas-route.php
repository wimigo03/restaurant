...
Route::prefix('empresas')->name('empresas.')->middleware(['auth:sanctum',config('jetstream.auth_session'),'verified',])->group(function () {
    Route::get('/{pi_cliente_id}', 'EmpresaController@index')->name('index')->middleware('can:empresas.index');
    Route::get('/search/{pi_cliente_id}', 'EmpresaController@search')->name('search')->middleware('can:empresas.index');
    Route::get('/create/{pi_cliente_id}', 'EmpresaController@create')->name('create')->middleware('can:empresas.create');
