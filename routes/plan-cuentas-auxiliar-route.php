...
Route::prefix('plan-cuentas-auxiliar')->name('plan_cuentas.auxiliar.')->middleware(['auth:sanctum',config('jetstream.auth_session'),'verified',])->group(function () {
    ...
    Route::get('/index/{empresa_id}', 'PlanCuentaAuxiliarController@index')->name('index')->middleware('can:plan.cuentas.auxiliar.index');
    Route::get('/search/{empresa_id}', 'PlanCuentaAuxiliarController@search')->name('search')->middleware('can:plan.cuentas.auxiliar.index');
    Route::get('/create/{empresa_id}', 'PlanCuentaAuxiliarController@create')->name('create')->middleware('can:plan.cuentas.auxiliar.create');
