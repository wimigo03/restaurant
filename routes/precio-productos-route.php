...
Route::prefix('precio-productos')->name('precio.productos.')->middleware(['auth:sanctum',config('jetstream.auth_session'),'verified',])->group(function () {
    ...
    Route::get('/', 'PrecioProductoController@index')->name('index')->middleware('can:precio.productos.index');
    Route::get('/search-tipo', 'PrecioProductoController@searchTipo')->name('search.tipo')->middleware('can:precio.productos.index');
    Route::get('/search-tipo-base', 'PrecioProductoController@searchTipoBase')->name('search.tipo.base')->middleware('can:precio.productos.index');
